...
const dummy = {
  ...
  date: "July 10, 2025",
  writerName: "Ravinder",
  description: `India’s startup ecosystem continues to buzz with energy and innovation...`,
};
...
const InsightNews = () => {
  ...
  const getInsightsNews = async () => {
    try {
      const res = await axios.get(
        `${backendURL}/api/insights/news`
      );
      setNews(res.data.news);
    } catch (err) {
      console.error("Error fetching insights news:", err.message);
    }
  };
        ...
        <div className="pl-1">
          ...
          <div className="w-full mt-4 bg-white rounded-md shadow p-4">
            ...
            <p className="text-sm text-slate-600">
              {parse(pulse.description).slice(0, 800)}...
            </p>
          </div>
        </div>
      </div>
