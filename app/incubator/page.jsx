...
const Incubator = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 mt-10">
      <h1 className="text-3xl font-bold mb-6">Incubator</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-10 w-full gap-2">
        {news?.length === 0 ? (
          <p className="text-center text-gray-500">

            <Image src={img2} alt="news not uploaded yet"/>
          </p>
        ) : (
          news.map((item) => (
            <Link key={item?.id} href={`/incubator/${item?.id}`}>
              <Card
