...
import Image from "next/image";
import img1 from "../../assets/News.png";
import img2 from "../../assets/Notupload.png"
const MSME = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 mt-10">
      <h1 className="text-3xl font-bold mb-6">MSME</h1>
      <div className="grid grid-cols-1 items-center sm:grid-cols-2 lg:grid-cols-3 mt-10 w-full gap-2">
        {news?.length === 0 ? (
          <p className="text-center text-gray-500">
            <Image className="flex justify-center items-center" src={img2} alt="image" />
          </p>
        ) : (
          news.map((item) => (
            <Link key={item?.id} href={`/msme/${item?.id}`}>
              <Card
